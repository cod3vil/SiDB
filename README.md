...
支持 **MySQL · MariaDB · PostgreSQL · SQL Server · SQLite · Redis · TDengine**
...
### 连接管理
- MySQL / MariaDB / PostgreSQL / SQL Server / SQLite / Redis / TDengine 七种数据源
...
### 数据导出
- 右键表 / 数据库：**转存结构** 或 **转存结构和数据** → `.sql`
- 结果集底部「导出」：格式 **CSV / XLSX / SQL**，范围 **全部 / 当前页 / 指定行数**
- **后台任务 + 进度浮层 + 可取消**；逐页流式写盘、不全量入内存，导出期间不阻塞同连接其它查询

### AI 助手
- 自然语言对话侧栏，多会话 + **历史抽屉**（可清空）
